...
const unsigned long long Random64[781] = {
...
unsigned long long board::hash() const
{
	...
	for (int i=0; i<BOARD_SQRS; i++)
	{
		switch (squares[i])
		{
		case PIECE_PAWN | PIECE_BLACK:
			result ^= Random64[i];
			break;
		case PIECE_PAWN | PIECE_WHITE:
			result ^= Random64[64+i];
			break;
		case PIECE_KNIGHT | PIECE_BLACK:
			result ^= Random64[64*2+i];
			break;
		case PIECE_KNIGHT | PIECE_WHITE:
			result ^= Random64[64*3+i];
			break;
		case PIECE_BISHOP | PIECE_BLACK:
			result ^= Random64[64*4+i];
			break;
		case PIECE_BISHOP | PIECE_WHITE:
			result ^= Random64[64*5+i];
			break;
		case PIECE_ROOK | PIECE_BLACK:
			result ^= Random64[64*6+i];
			break;
		case PIECE_ROOK | PIECE_WHITE:
			result ^= Random64[64*7+i];
			break;
		case PIECE_QUEEN | PIECE_BLACK:
			result ^= Random64[64*8+i];
			break;
		case PIECE_QUEEN | PIECE_WHITE:
			result ^= Random64[64*9+i];
			break;
		case PIECE_KING | PIECE_BLACK:
			result ^= Random64[64*10+i];
			break;
		case PIECE_KING | PIECE_WHITE:
			result ^= Random64[64*11+i];
			break;
		} 
	}
	...
	if (enPassantColumn != 0)
	{
		int enPassantSquare = (enPassantColumn - 1) + (toMove?3:4)*8;
		if ((enPassantColumn != 1 && squares[enPassantSquare-1] == (PIECE_PAWN | toMove))
		 || (enPassantColumn != 8 && squares[enPassantSquare+1] == (PIECE_PAWN | toMove)))
			result ^= Random64[771+enPassantColumn];
	}
